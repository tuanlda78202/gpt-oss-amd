...
float* forward(OssTransformerHybrid* transformer, OssExpertParallelGroup* ep_group, int* tokens,
    ...
    for (unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        std::vector<int> wq_heavy_all = make_bucket_wq(heavy_ids);
        std::vector<int> wq_light_all = make_bucket_wq(light_ids);
        ...
        if (!ep_group || ep_group->ep_size <= 1 || transformer->ep_local_experts == 0) {
            ...
            if (!wq_heavy_all.empty()) {
                assert(d_wq_heavy && (size_t)wq_heavy_all.size() <= (size_t)s->d_wq_heavy_capacity);
                CHECK_HIP(hipMemcpyAsync(d_wq_heavy, wq_heavy_all.data(),
                                         (size_t)wq_heavy_all.size() * sizeof(int),
                                         hipMemcpyHostToDevice, tls.h2d));
            ...
            if (!wq_light_all.empty()) {
                assert(d_wq_light && (size_t)wq_light_all.size() <= (size_t)s->d_wq_light_capacity);
                CHECK_HIP(hipMemcpyAsync(d_wq_light, wq_light_all.data(),
                                         (size_t)wq_light_all.size() * sizeof(int),
                                         hipMemcpyHostToDevice, tls.h2d));
            ...
            CHECK_HIP(hipMemset(s->e_agg, 0, e_agg_bytes));
            ...
            for (int shard_idx = 0; shard_idx < ep_size; ++shard_idx) {
                ...
                if (work.total > 0) {
                    ...
                    for (size_t i = 0; i < work.experts.size(); ++i) {
                        ...
                        CHECK_HIP(hipMemcpyPeerAsync(workspace->tokens_by_expert + local_offset,
                                                     ...
                                                     primary_device, index_bytes, remote_stream));

                        CHECK_HIP(hipMemcpyPeerAsync(workspace->weights_by_expert + local_offset,
                                                     remote_device, s->weights_flat + goff,
                                                     primary_device, weight_bytes, remote_stream));

                        local_offset += cnt;
                    }
                }

                const size_t agg_bytes = e_agg_bytes;
                if (agg_bytes > 0)
                    CHECK_HIP(hipMemsetAsync(workspace->e_by_token, 0, agg_bytes, remote_stream));
                ...
                moe_mlp2_scatter(
                    ...
                    workspace->tokens_by_expert, workspace->weights_by_expert,
                    workspace->e_by_token,
                ...
                remote_tasks.push_back({shard_idx, shard, completion_event, agg_bytes,
                                        remote_device, workspace, mlp1_ms_remote, mlp2_ms_remote,
                                        0.0f});
            ...
            for (size_t task_idx = 0; task_idx < remote_tasks.size(); ++task_idx) {
                ...
                hipStream_t agg_stream = tls.compute;
                hipEvent_t agg_event = nullptr;
                float* agg_buffer = s->y_by_expert;

                if (task.shard_index < ep_size) {
                    if (agg_streams && agg_streams[task.shard_index])
                        agg_stream = agg_streams[task.shard_index];
                    if (agg_events && agg_events[task.shard_index])
                        agg_event = agg_events[task.shard_index];
                    if (agg_buffers && agg_buffers[task.shard_index])
                        agg_buffer = agg_buffers[task.shard_index];
                }

                CHECK_HIP(hipMemcpyPeerAsync(agg_buffer, primary_device, task.workspace->e_by_token,
                                             task.remote_device, task.agg_bytes, agg_stream));

                if (agg_event) {
                    CHECK_HIP(hipEventRecord(agg_event, agg_stream));
                    if (agg_stream != tls.compute)
                        CHECK_HIP(hipStreamWaitEvent(tls.compute, agg_event, 0));
                } else if (agg_stream != tls.compute) {
                ...
                vecadd(s->e_agg, agg_buffer, 1.0f, batch_size, hidden_dim, tls.compute);
            }
            CHECK_HIP(hipSetDevice(primary_device));
