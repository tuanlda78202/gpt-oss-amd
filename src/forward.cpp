...
float* forward(OssTransformerHybrid* transformer, int* tokens, const int* pos_per_token_h,
    ...
    for (unsigned long long l = 0; l < p->n_layers; l++) {
        ...
        int *d_wq_heavy = nullptr, *d_wq_light = nullptr;
        if (!wq_heavy_h.empty()) {
            CHECK_HIP(hipMalloc(&d_wq_heavy, (int)wq_heavy_h.size() * sizeof(int)));
            CHECK_HIP(hipMemcpy(d_wq_heavy, wq_heavy_h.data(), (int)wq_heavy_h.size() * sizeof(int),
                                hipMemcpyHostToDevice));
        }
        if (!wq_light_h.empty()) {
            CHECK_HIP(hipMalloc(&d_wq_light, (int)wq_light_h.size() * sizeof(int)));
            CHECK_HIP(hipMemcpy(d_wq_light, wq_light_h.data(), (int)wq_light_h.size() * sizeof(int),
                                hipMemcpyHostToDevice));
        }
        ...
        if (heavy_count > 0) {
            moe_mlp1_swiglu(d_wq_heavy, /*work_start*/ 0, /*work_count*/ heavy_count,
                            ...
                            p->swiglu_limit, rh_heavy_mlp1, kGridCap, 0);
        ...
        if (light_count > 0) {
            moe_mlp1_swiglu(d_wq_light, /*work_start*/ 0, /*work_count*/ light_count,
                            ...
                            w->b_mlp1 + 1ll * l * p->n_experts * mlp1_bias_stride, hidden_dim,
                            p->intermediate_dim, mlp1_weight_stride, mlp1_bias_stride, alpha,
                            p->swiglu_limit, rh_light_mlp1, kGridCap, 0);
            ...
            moe_mlp2_scatter(d_wq_light, /*work_start*/ 0, /*work_count*/ light_count,
                             ...
                             w->b_mlp2 + 1ll * l * p->n_experts * mlp2_bias_stride,
                             p->intermediate_dim, hidden_dim, mlp2_weight_stride, mlp2_bias_stride,
                             rh_light_mlp2, kGridCap, 0);
        }

        // free bucket queues
        if (d_wq_heavy)
            CHECK_HIP(hipFree(d_wq_heavy));
        if (d_wq_light)
            CHECK_HIP(hipFree(d_wq_light));
