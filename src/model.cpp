...
│ OssTransformer                      │
│ ├── config (small)                  │
│ ├── weights (huge - 20/120B params) │
│ └── state (medium - temp buffers)   │
└─────────────────────────────────────┘
*/
...
void copy_transformer_to_device_half(OssTransformerHalf* t_h, OssTransformerHalf* t_d) {
    ...
    // ! GPU Memory Allocation for Weights
    CHECK_HIP(
        hipMalloc(&t_d->weights.token_embedding_table, vocab_size * hidden_dim * sizeof(__half)));
    ...
    CHECK_HIP(
        hipMalloc(&t_d->weights.w_router, n_layers * hidden_dim * n_experts * sizeof(__half)));
    ...
    // ! Copy Weights to GPU
    CHECK_HIP(hipMemcpy(t_d->weights.token_embedding_table, weights->token_embedding_table,
                        vocab_size * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.rms_attn_w, weights->rms_attn_w,
                        n_layers * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.rms_ffn_w, weights->rms_ffn_w,
                        n_layers * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.w_qkv, weights->w_qkv,
                        n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) *
    ...
    CHECK_HIP(
        hipMemcpy(t_d->weights.b_qkv, weights->b_qkv,
                  n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) * sizeof(__half),
                  hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_o, weights->b_o, n_layers * hidden_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.attn_sinks, weights->attn_sinks,
                        n_layers * n_attn_heads * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.w_router, weights->w_router,
                        n_layers * hidden_dim * n_experts * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_router, weights->b_router,
                        n_layers * n_experts * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(
        hipMemcpy(t_d->weights.w_mlp1, weights->w_mlp1,
    ...
    CHECK_HIP(hipMemcpy(t_d->weights.b_mlp1, weights->b_mlp1,
                        ...
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_mlp2, weights->b_mlp2,
                        n_layers * n_experts * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.rms_out_w, weights->rms_out_w, hidden_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.out, weights->out, vocab_size * hidden_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    ...
    CHECK_HIP(
        hipMalloc(&t_d->state.qkv, head_dim * (n_attn_heads + 2 * n_kv_heads) * sizeof(__half)));
    ...
    CHECK_HIP(
        hipMalloc(&t_d->state.mask, seq_len * sizeof(__half))); // TODO: check if this is correct

    CHECK_HIP(
        hipMemcpy(t_d->state.x, state->x, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(
        hipMemcpy(t_d->state.t, state->t, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.tb, state->tb, head_dim * n_attn_heads * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(
        hipMemcpy(t_d->state.tb2, state->tb2, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.router_score, state->router_score, n_experts * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.topk_v, state->topk_v, experts_per_token * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.topk_i, state->topk_i, experts_per_token * sizeof(int),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.mlp1_out, state->mlp1_out, hidden_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.gate, state->gate, hidden_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(
        hipMemcpy(t_d->state.up, state->up, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.gate_up, state->gate_up, hidden_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.e_agg, state->e_agg, hidden_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.qkv, state->qkv,
                        head_dim * (n_attn_heads + 2 * n_kv_heads) * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.q, state->q, n_attn_heads * head_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.k, state->k, n_kv_heads * head_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.v, state->v, n_kv_heads * head_dim * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.att, state->att, n_attn_heads * seq_len * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.logits, state->logits, vocab_size * sizeof(__half),
                        hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.key_cache, state->key_cache,
                        n_layers * seq_len * head_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.value_cache, state->value_cache,
                        n_layers * seq_len * head_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(
        hipMemcpy(t_d->state.mask, state->mask, seq_len * sizeof(__half), hipMemcpyHostToDevice));
}
