...
void copy_transformer_to_device_half(OssTransformerHalf* t_h, OssTransformerHalf* t_d) {
    OssConfig* conf = &t_h->config;
    OssTransformerWeightsHalf* weights = &t_h->weights;
    OssRunStateHalf* state = &t_h->state;
    ...
    printf("token_embedding_table: %lu\n", vocab_size * hidden_dim * sizeof(__half));
    printf("rms_attn_w: %lu\n", n_layers * hidden_dim * sizeof(__half));
    printf("rms_ffn_w: %lu\n", n_layers * hidden_dim * sizeof(__half));
    printf("w_qkv: %lu\n", n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) * hidden_dim * sizeof(__half));
    printf("w_o: %lu\n", n_layers * hidden_dim * head_dim * n_attn_heads * sizeof(__half));
    printf("b_qkv: %lu\n", n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) * sizeof(__half));
    printf("b_o: %lu\n", n_layers * hidden_dim * sizeof(__half));
    printf("attn_sinks: %lu\n", n_layers * n_attn_heads * sizeof(__half));
    printf("w_router: %lu\n", n_layers * hidden_dim * n_experts * sizeof(__half));
    printf("b_router: %lu\n", n_layers * n_experts * sizeof(__half));
    printf("w_mlp1: %lu\n", n_layers * n_experts * 2 * intermediate_dim * hidden_dim * sizeof(__half));
    printf("w_mlp2: %lu\n", n_layers * n_experts * hidden_dim * intermediate_dim * sizeof(__half));
    printf("b_mlp1: %lu\n", n_layers * n_experts * 2 * intermediate_dim * sizeof(__half));
    printf("b_mlp2: %lu\n", n_layers * n_experts * hidden_dim * sizeof(__half));
    printf("rms_out_w: %lu\n", hidden_dim * sizeof(__half));
    printf("out: %lu\n", vocab_size * hidden_dim * sizeof(__half));

    CHECK_HIP(hipMalloc(&t_d->weights.token_embedding_table, vocab_size * hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.rms_attn_w, n_layers * hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.rms_ffn_w, n_layers * hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.w_qkv, n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) * hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.w_o, n_layers * hidden_dim * head_dim * n_attn_heads * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.b_qkv, n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.b_o, n_layers * hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.attn_sinks, n_layers * n_attn_heads * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.w_router, n_layers * hidden_dim * n_experts * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.b_router, n_layers * n_experts * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.w_mlp1, n_layers * n_experts * 2 * intermediate_dim * hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.w_mlp2, n_layers * n_experts * hidden_dim * intermediate_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.b_mlp1, n_layers * n_experts * 2 * intermediate_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.b_mlp2, n_layers * n_experts * hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.rms_out_w, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->weights.out, vocab_size * hidden_dim * sizeof(__half)));

    CHECK_HIP(hipMemcpy(t_d->weights.token_embedding_table, weights->token_embedding_table, vocab_size * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.rms_attn_w, weights->rms_attn_w, n_layers * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.rms_ffn_w, weights->rms_ffn_w, n_layers * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.w_qkv, weights->w_qkv, n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.w_o, weights->w_o, n_layers * hidden_dim * head_dim * n_attn_heads * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_qkv, weights->b_qkv, n_layers * (head_dim * n_attn_heads + 2 * head_dim * n_kv_heads) * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_o, weights->b_o, n_layers * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.attn_sinks, weights->attn_sinks, n_layers * n_attn_heads * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.w_router, weights->w_router, n_layers * hidden_dim * n_experts * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_router, weights->b_router, n_layers * n_experts * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.w_mlp1, weights->w_mlp1, 1ll * n_layers * n_experts * 2 * intermediate_dim * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.w_mlp2, weights->w_mlp2, n_layers * n_experts * hidden_dim * intermediate_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_mlp1, weights->b_mlp1, n_layers * n_experts * 2 * intermediate_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.b_mlp2, weights->b_mlp2, n_layers * n_experts * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.rms_out_w, weights->rms_out_w, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->weights.out, weights->out, vocab_size * hidden_dim * sizeof(__half), hipMemcpyHostToDevice));

    // Copy state to device
    printf("Copying state to device\n");
    CHECK_HIP(hipMalloc(&t_d->state.x, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.t, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.tb, head_dim * n_attn_heads * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.tb2, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.router_score, n_experts * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.topk_v, experts_per_token * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.topk_i, experts_per_token * sizeof(int)));
    CHECK_HIP(hipMalloc(&t_d->state.mlp1_out, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.gate, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.up, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.gate_up, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.e_agg, hidden_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.qkv, head_dim * (n_attn_heads + 2 * n_kv_heads) * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.q, n_attn_heads * head_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.k, n_kv_heads * head_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.v, n_kv_heads * head_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.att, n_attn_heads * seq_len * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.logits, vocab_size * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.key_cache, n_layers * seq_len * head_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.value_cache, n_layers * seq_len * head_dim * sizeof(__half)));
    CHECK_HIP(hipMalloc(&t_d->state.mask, seq_len * sizeof(__half)));  // TODO: check if this is correct

    CHECK_HIP(hipMemcpy(t_d->state.x, state->x, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.t, state->t, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.tb, state->tb, head_dim * n_attn_heads * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.tb2, state->tb2, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.router_score, state->router_score, n_experts * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.topk_v, state->topk_v, experts_per_token * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.topk_i, state->topk_i, experts_per_token * sizeof(int), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.mlp1_out, state->mlp1_out, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.gate, state->gate, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.up, state->up, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.gate_up, state->gate_up, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.e_agg, state->e_agg, hidden_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.qkv, state->qkv, head_dim * (n_attn_heads + 2 * n_kv_heads) * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.q, state->q, n_attn_heads * head_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.k, state->k, n_kv_heads * head_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.v, state->v, n_kv_heads * head_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.att, state->att, n_attn_heads * seq_len * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.logits, state->logits, vocab_size * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.key_cache, state->key_cache, n_layers * seq_len * head_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.value_cache, state->value_cache, n_layers * seq_len * head_dim * sizeof(__half), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(t_d->state.mask, state->mask, seq_len * sizeof(__half), hipMemcpyHostToDevice));
}

void free_transformer_on_device_half(OssTransformerHalf* t_d) {
    printf("Freeing weights on device\n");
    CHECK_HIP(hipFree(t_d->weights.token_embedding_table));
    CHECK_HIP(hipFree(t_d->weights.rms_attn_w));
    CHECK_HIP(hipFree(t_d->weights.rms_ffn_w));
    CHECK_HIP(hipFree(t_d->weights.w_qkv));
    ...
    printf("Freeing state on device\n");
    CHECK_HIP(hipFree(t_d->state.x));
    CHECK_HIP(hipFree(t_d->state.t));
    CHECK_HIP(hipFree(t_d->state.tb));
    CHECK_HIP(hipFree(t_d->state.tb2));
