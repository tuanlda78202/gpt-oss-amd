    ...
    if (g_ep_size > 0) {
        ...
        for (int ep = 0; ep < g_ep_size; ++ep) {
            ...
            for (int dp = 0; dp < shard.workspace_count; ++dp) {
                ...
                CHECK_HIP(hipStreamCreateWithFlags(&shard.streams[dp], hipStreamNonBlocking));
            }
        }
    }
    ...
    if (g_expert_shards) {
        for (int ep = 0; ep < g_ep_size; ++ep) {
            ...
            for (int dp = 0; dp < shard->workspace_count; ++dp) {
                ...
                if (ws.e_by_token)
                    CHECK_HIP(hipFree(ws.e_by_token));
                if (ws.work_queue)
                    CHECK_HIP(hipFree(ws.work_queue));

                if (shard->streams && shard->streams[dp])
                    CHECK_HIP(hipStreamDestroy(shard->streams[dp]));
...
long long generate(Transformer* transformer, Tokenizer* tokenizer, Sampler* sampler,
    ...
    while (active_sequences > 0) {
        ...
        if (use_async_sampling && pending_samples > 0) {
            static thread_local hipStream_t sampling_stream = nullptr;
            static thread_local hipEvent_t sampling_done = nullptr;
            if (!sampling_stream)
                CHECK_HIP(hipStreamCreateWithFlags(&sampling_stream, hipStreamNonBlocking));
            if (!sampling_done)
                CHECK_HIP(hipEventCreateWithFlags(&sampling_done, hipEventDisableTiming));

            if (logits_ready_event)
                CHECK_HIP(hipStreamWaitEvent(sampling_stream, logits_ready_event, 0));
            ...
            CHECK_HIP(hipMemcpyAsync(sample_results_h, sample_results_d,
                                     ...
                                     sampling_stream));
            CHECK_HIP(hipEventRecord(sampling_done, sampling_stream));
            CHECK_HIP(hipStreamWaitEvent(0, sampling_done, 0));
            CHECK_HIP(hipEventSynchronize(sampling_done));
            for (int s = 0; s < pending_samples; ++s) {
