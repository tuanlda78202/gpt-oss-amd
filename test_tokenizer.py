...
def run_cmd(cmd: List[str], timeout: float) -> str:
    try:
        ...
        print(f"[ERROR] Command failed ({e.returncode}): {shlex.join(cmd)}",
              file=sys.stderr)
        if e.output:
...
def run_c_encoder(binary: Path, tokbin: Path, text: str,
...
def run_c_decoder(binary: Path, tokbin: Path, text: str,
                  timeout: float) -> str:
...
def first_diff(a: List[int],
               b: List[int]) -> Tuple[int, int | None, int | None]:
    n = min(len(a), len(b))
    for i in range(n):
        if a[i] != b[i]:
            return i, a[i], b[i]
    if len(a) != len(b):
        return n, (a[n] if n < len(a) else None), (b[n]
                                                   if n < len(b) else None)
    return -1, None, None
...
def compare_one(binary: Path, tokbin: Path, enc: tiktoken.Encoding,
                prompt: str, timeout: float) -> CaseResult:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--bin",
                   required=True,
                   help="Path to C test_tokenizer binary")
    ...
    p.add_argument("--only-mismatches",
                   action="store_true",
...
def main() -> None:
    ...
    try:
        ...
        print(f"[ERROR] Unknown tiktoken encoding '{args.encoding}': {e}",
              file=sys.stderr)
        sys.exit(2)
    ...
    for prompt in prompts:
        ...
        should_print = args.verbose or (
            args.only_mismatches and is_mismatch) or (not args.only_mismatches)
        ...
        if res.encode_match:
            if should_print: print("  [ENCODE MATCH]")
        else:
            enc_miss += 1
        ...
        if res.decode_match:
            if should_print: print("  [DECODE MATCH]")
        else:
            dec_miss += 1
            if should_print:
